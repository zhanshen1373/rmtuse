					...
					while (iterTr.hasNext()) {
						...
						{ // 查找主表的记录主键索引的HashMap
							...
							try{
								...
								attributeObj=objParent.getJSONObject(0)
								.get(tableName);
							}
							catch(Exception e)
							{}
							if (attributeObj != null) {
								objParent.getJSONObject(0).getJSONArray(tableName).put(entryTr.getValue().get(0));
							}
							else{
								objParent.getJSONObject(0).put(tableName,entryTr.getValue());
							}
							isRemove = true;
						}
					}
